...
case class RedisDelAction(ctx: ScenarioContext, next: Action, clientPool: RedisClientPool, key: Expression[Any], keys: Seq[Expression[Any]])
  ...
  override def execute(session: Session): Unit = {
    try {
      ...
      } yield redisDeleteKey(resolvedKey, resolvedKeys).success
      next ! session
    } catch {
      case e: Exception =>
