...
case class RedisSremAction(ctx: ScenarioContext, next: Action, clientPool: RedisClientPool, key: Expression[Any], value: Expression[Any], values: Seq[Expression[Any]])
  ...
  override def execute(session: Session): Unit = {
    try {
      ...
      } yield deleteKeyMember(resolvedKey, resolvedVal, resolvedVals).success
      next ! session
    } catch {
      case e: Exception =>
